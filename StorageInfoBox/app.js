...
const DataVault = (function ($) {
  ...
  const STORAGE_KEY = "infoandstorage_data";
  const THEME_KEY = "infoandstorage_theme";
  const WARN_DAYS = 30;
  ...
  function itemCount(secId) {
    return state.items.filter((i) => i.sectionId === secId).length;
  }
  ...
  function getDisplayItems() {
    ...
    if (activeTagFilters.length) {
      items = items.filter((i) =>
        activeTagFilters.every((t) => (i.tags || []).includes(t))
      );
    }
  ...
  function updateFilterBadge() {
    ...
    if (n > 0) {
      $b.text(n).removeClass("hidden");
      $("#advanced-filter-btn").addClass("active");
    } else {
      $b.addClass("hidden");
  ...
  function renderTagChips() {
    ...
    allTags().forEach((t) => {
      ...
      $wrap.append(
        `<span class="tag-chip ${sel ? "selected" : ""}" data-tag="${esc(
          t
        )}">${esc(t)}</span>`
      );
    });
  ...
  function renderItems() {
    ...
    $("#section-title").text(
      globalSearch ? "Search Results" : sec?.name || "—"
    );
    $("#item-count").text(
  ...
  function renderCardView(items, globalSearch, q) {
    ...
    if (globalSearch) {
      const grouped = {};
      items.forEach((i) => {
        if (!grouped[i.sectionId]) grouped[i.sectionId] = [];
        grouped[i.sectionId].push(i);
      });
      Object.entries(grouped).forEach(([sid, grp]) => {
        const s = secById(sid);
        $("#items-grid").append(
          `<div class="search-section-header">${esc(
  ...
  function buildCard(item, q) {
    ...
    let notesHtml = item.notes
      ? `<div class="card-notes">${esc(item.notes)}</div>`
      : "";
    let tagsHtml = (item.tags || [])
      .map((t) => `<span class="tag">${esc(t)}</span>`)
      .join("");
    let expiryHtml = status
  ...
  function renderExpiryBanner() {
    const exp = state.items.filter(
      (i) => expiryStatus(i.expiry)?.cls === "expired"
    ).length;
    const warn = state.items.filter(
      (i) => expiryStatus(i.expiry)?.cls === "warn"
    ).length;
  ...
  function addDynField(key = "", value = "", sensitive = false) {
    ...
    $("#dynamic-fields").append(`
      <div class="dyn-field-row" id="dfr-${id}">
        <input type="text" class="fk-input" placeholder="Field name" value="${esc(
          key
        )}">
        <input type="text" class="fv-input" placeholder="Value" value="${esc(
          value
        )}">
        <label class="sens-label" title="Mask this field">
          <input type="checkbox" class="sens-check" ${
            sensitive ? "checked" : ""
          }>
          mask
        </label>
  ...
  function openItemModal(itemId = null) {
    ...
    (item?.fields || []).forEach((f) =>
      addDynField(f.key, f.value, f.sensitive)
    );
  ...
  function saveBulkItems() {
    ...
    if (added === 0) {
      toast("No valid rows to add", "error");
      return;
    }
    saveData();
    renderSidebar();
  ...
  function deleteSection(secId) {
    ...
    showConfirm(
      "Delete Section",
      cnt > 0
        ? `This section has ${cnt} item(s) — all will be deleted. Continue?`
        : "Delete this section?",
      () => {
  ...
  function doExport() {
    ...
    const items =
      scope === "all"
        ? state.items
        : state.items.filter((i) => i.sectionId === scope);
    const sections =
      scope === "all"
        ? state.sections
        : state.sections.filter((s) => s.id === scope);

    if (fmt === "json") {
      downloadBlob(
        new Blob([JSON.stringify({ sections, items }, null, 2)], {
      ...
      items.forEach((i) => {
        ...
        rows.push([
          ...
          i.title,
          s?.name || "",
          (i.fields || []).map((f) => `${f.key}:${f.value}`).join("|"),
          (i.tags || []).join(", "),
          i.expiry || "",
          (i.notes || "").replace(/\n/g, " "),
    ...
    Object.entries(grouped).forEach(([sid, grp]) => {
      ...
      grp.forEach((item) => {
        ...
        let flds = (item.fields || [])
          .map(
            (f) =>
              `<div class="c-field"><span class="c-key">${esc(
                f.key
              )}</span><span class="c-val">${
                f.sensitive ? "••••••" : esc(f.value)
              }</span></div>`
          )
          .join("");
        let notes = item.notes
          ? `<div class="c-notes">${esc(item.notes)}</div>`
          : "";
        let tags = (item.tags || [])
          .map((t) => `<span class="tag">${esc(t)}</span>`)
          .join("");
        let expiry = st
          ? `<span class="expiry ${st.cls}">${esc(st.label)}</span>`
          : "";
        html += `<div class="card ${cls}"><div class="c-title">${esc(
          item.title
        )}</div>${flds}${notes}<div class="c-footer"><div class="tags">${tags}</div>${expiry}</div></div>`;
      });
      html += `</div>`;
    });
    html += `</body></html>`;
  ...
  function validateImport(data) {
    ...
    $("#import-preview")
      .removeClass("hidden")
      .html(
        `<strong>Ready to import:</strong><br>${secs.length} section(s) · ${items.length} item(s)`
      );
  ...
  function doImport() {
    ...
    importBuffer.items.forEach((imp) => {
      ...
      if (ex) {
        Object.assign(ex, imp, { updatedAt: Date.now() });
        updItems++;
      } else {
        state.items.unshift(imp);
        newItems++;
      }
    });
    saveData();
    ...
    closeModals();
    toast(
      `Imported: ${newItems} added, ${updItems} updated, ${newSecs} sections`,
      "success",
      4000
    );
  }
  ...
  function bindEvents() {
    ...
    $(document).on("click", ".edit-sec", function (e) {
      e.stopPropagation();
      openSectionModal($(this).data("sec-id"));
    });
    $(document).on("click", ".del-sec", function (e) {
      e.stopPropagation();
      deleteSection($(this).data("sec-id"));
    });
    $("#add-section-btn").on("click", () => openSectionModal());
    $("#section-save-btn").on("click", saveSectionModal);
    $("#section-name-input").on("keydown", (e) => {
    ...
    // Bulk add
    $("#add-bulk-row-btn").on("click", () =>
      addBulkRow($("#bulk-rows tr").length + 1)
    );
    $(document).on("click", ".bulk-del-btn", function () {
      $(`#br-${$(this).data("row")}`).remove();
    });
    ...
    $(document).on("click", ".dup-btn", function (e) {
      e.stopPropagation();
      duplicateItem($(this).data("id"));
    });
    $(document).on("click", ".pin-btn", function (e) {
      e.stopPropagation();
      togglePin($(this).data("id"));
    });
    ...
    $("#reorder-btn").on("click", function () {
      ...
      renderItems();
      toast(
        reorderMode ? "Drag cards to reorder" : "Reorder off",
        "info",
        1600
      );
    });
    ...
    $("#global-search").on("input", function () {
      if ($(this).val()) $("#section-search").val("");
      renderItems();
    });
    $("#section-search").on("input", () => {
      renderItems();
      updateFilterBadge();
    });
    ...
    $(document).on(
      "click",
      ".modal-close, .modal-cancel, .modal-overlay",
      closeModals
    );
