...
const QuestionRenderer = (() => {
  ...
  function render(panel, q, idx) {
    ...
    const diffColor = { easy:'var(--color-success)', medium:'var(--color-warn)', hard:'var(--color-error)' };
    ...
    panel.innerHTML = `
      ...
      <div class="q-right-panel animate-fade" style="display:flex;flex-direction:column;gap:var(--sp-lg)">
        ...
        <div style="display:flex;gap:6px;flex-wrap:wrap;align-items:center">
          ...
          ${q['Sub Category'] ? `<span class="badge badge-neutral">${q['Sub Category']}</span>` : ''}
          ${q.Difficulty ? `<span class="badge" style="background:${dc}22;color:${dc}">${q.Difficulty}</span>` : ''}
          <span class="badge badge-neutral">${_currentType}</span>
          ${q.Score ? `<span class="badge badge-neutral">+${q.Score} pts</span>` : ''}
  ...
  function renderQuestionText(q) {
    ...
    if (_currentType === 'Inline Blank') {
      text = text.replace(/\[blank\]/gi, '<input class="inline-blank" type="text" placeholder="…" style="border:none;border-bottom:2px solid var(--accent-primary);background:transparent;color:var(--text-primary);font-family:var(--font-ui);font-size:1rem;width:120px;outline:none;padding:2px 4px;">');
    }
    return text;
  }
  ...
  function renderAnswerArea(q, saved) {
    ...
    switch(_currentType) {
      ...
      case 'Short Answer':
        return `<textarea id="short-ans" class="form-control" rows="4" placeholder="Type your answer here…"
    ...
    return `
      <div id="mcq-options" style="display:flex;flex-direction:column;gap:var(--sp-sm)">
        ${choices.map((c, i) => {
          ...
          const correctArr  = (quiz.questions[_currentIdx]['Correct Answer'] || '').split('|');
          const isCorrect   = correctArr.includes(c);
          let extraClass = isSel ? 'selected' : '';
          if (showInstant && isSel) extraClass = isCorrect ? 'correct' : 'wrong';
          return `
            <div class="option-card ${extraClass}" data-val="${c}" onclick="QuestionRenderer.toggleMCQ(this,'${multi}')">
              ...
              ${showInstant && isSel ? `<span>${isCorrect ? '✓' : '✕'}</span>` : ''}
            </div>`;
        }).join('')}
      </div>`;
  }
  ...
  function renderTrueFalse(saved) {
    return `
      <div style="display:flex;gap:var(--sp-md)">
        ${['TRUE','FALSE'].map(v => `
          <div class="option-card ${saved===v?'selected':''}" style="flex:1;justify-content:center;font-size:1.1rem;font-weight:700"
  ...
  function renderFillBlanks(q, saved) {
    return `
      <div class="form-group">
        ...
        <input id="fill-ans" class="form-control" type="text" placeholder="Type answer…" value="${saved||''}">
      </div>`;
  }
      ...
      <div style="display:grid;grid-template-columns:1fr auto 1fr;gap:var(--sp-sm);align-items:center">
        ${lefts.map((l, i) => `
          ...
          <select class="form-control match-select" data-left="${l}">
            ...
            ${shuffled.map(r => `<option value="${r}" ${savedMap[l]===r?'selected':''}>${r}</option>`).join('')}
          </select>`).join('')}
      </div>`;
  }
  ...
  function renderMultiMatching(choices, correctStr, saved) {
    ...
    const rows = choices.map(c => {
      const parts = c.split('-');
      return { left: parts[0], tags: parts.slice(1) };
    });
    ...
    return `
      <div style="overflow-x:auto">
        <table class="data-table">
          <thead><tr><th>Item</th>${allTags.map(t=>`<th>${t}</th>`).join('')}</tr></thead>
          <tbody>
            ${rows.map(r=>`
              <tr>
  ...
  function renderDragDrop(choices, saved) {
    const categories = [...new Set(choices.map(c => c.split('-')[1]).filter(Boolean))];
    return `
  ...
  function collectAnswer() {
    ...
    try {
      if (type === 'Multichoice' || type === 'True/False' || type === 'Multichoice Anycorrect') {
        const sel = document.querySelector('#mcq-options .option-card.selected, [data-val].selected');
        return sel ? sel.dataset.val : null;
      }
      if (type === 'Multi Multichoice') {
        const sels = [...document.querySelectorAll('#mcq-options .option-card.selected')];
      ...
      if (type === 'Inline Blank') {
        return [...document.querySelectorAll('.inline-blank')].map(i=>i.value).join('|') || null;
      }
      if (type === 'Multi Blanks') {
        const vals = [...document.querySelectorAll('.multi-blank')].map(i=>i.value);
      ...
      if (type === 'Drag & Drop') {
        ...
        document.querySelectorAll('.drag-target').forEach(tgt => {
          tgt.querySelectorAll('[data-val]').forEach(el => result.push(el.dataset.val));
        });
        return result.join('|');
      }
      if (type === 'Range') {
  ...
  function toggleMCQ(el, multiStr) {
    ...
    if (noChange && answered && answered.userAnswer) {
      UI.toast('Option change not allowed', 'warn'); return;
    }
    const multi = multiStr === 'true';
    if (!multi) {
      document.querySelectorAll('#mcq-options .option-card').forEach(c=>c.classList.remove('selected'));
    }
    ...
    if (dontChange) {
      ...
      if (el.classList.contains('selected') && !correct.includes(el.dataset.val)) {
    ...
    if ((cfg['Instant Answer'] || 'Off') === 'On') {
      ...
      document.querySelectorAll('#mcq-options .option-card').forEach(card => {
        ...
        if (card.classList.contains('selected')) {
          card.classList.add(correct.includes(card.dataset.val) ? 'correct' : 'wrong');
        }
      });
    }
...
const SeqDrag = {
  _el: null,
  start(e) { this._el = e.currentTarget; e.dataTransfer.effectAllowed='move'; },
...
const DragDrop = {
  ...
  over(e) { e.preventDefault(); e.currentTarget.style.borderColor='var(--accent-primary)'; },
  drop(e) {
    e.preventDefault();
